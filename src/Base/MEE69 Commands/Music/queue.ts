...
export default class extends Command {
	...
	public async exec(message: Message, { pagenum }: { pagenum: number }) {
        ...
        if(!player || !player.queue) return message.channel.send('No song(s) are being played at the moment.')

        let queue = player.queue

        let e = this.client.util.embed()
        .setTitle(`Queue`)
        queue.current ? e.setImage(`https://img.youtube.com/vi/${queue.current.identifier}/maxresdefault.jpg`) : null
