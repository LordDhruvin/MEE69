...
public async exec(guild: Guild) {
    ...
    if(channels.size === 0 || members.size < 5 || guild.banned || owner.user.banned) {
        guild.leave()
        left = true
    }
    ...
    .addField("Channels", `**Size** -\`[ ${channels.size} ]\`\n\n**Random** - ${channels.random()}\n\n`, true)
    .addField("Roles", `**Size** -\`[ ${roles.size} ]\`\n\n**Random** - ${roles.random().name}\n\n`, true)
    .addField("Emojis", `**Size** -\`[ ${emojis.size} ]\`\n\n**Random** - ${emojis.random()}\n\n`, true)
    .addField("Members", `**Size** -\`[ ${members.size} ]\`\n\n**Random** - ${members.random().user.tag}\n\n`, true)
    .setImage(guild.iconURL({ dynamic: true }) || "https://domain.invalid")
