...
export default class extends Command {
    ...
    public async exec(message: Message, { toReload, all}: { toReload: Command|Inhibitor|Listener, all: boolean}) {
        if(all) {
            ...
            toReload.reload()
            return message.util?.send(`\`${toReload.id}\` reloaded successfully`)
        } else {
