...
export default class extends Listener {
    public constructor() {
        super('listener_brawl_stars_command_handler_error', {
    ...
    public async exec(error: Error, message: Message, command: Command) {
        ...
        if(this.client.isOwner(message.author)){
            return message.channel.send(this.client.util.embed().setColor(0xff0000).addField(`Error`, `\`\`\`prolog\n${error.message}\n\`\`\``).addField(`Command`, `\`${command.id}\``))
        } else {
            ...
            .addField(`Message`, `\`${message.content ? Util.escapeMarkdown(message.content) : ' '}\`\n**Author:** ${message.author} \`${message.author.id}\`\n**Guild:** ${message.guild ?  `${message.guild.name} \`${message.guild.id}\`` : `DMs`}`)
