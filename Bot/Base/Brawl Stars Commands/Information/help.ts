...
export default class extends Command {
    ...
    public async exec(message: Message, { command }: { command: Command}) {
        ...
        for (let category of this.handler.categories.values()) {
            if(this.client.isOwner(message.author)) embed2.addField(`${category.id} \`[ ${category.size} ]\``, `${category.map((cmd) => `\`${cmd.aliases[0]}\``).join(', ')}`)
            else if(category.id != 'Owner') embed2.addField(`${category.id} \`[ ${category.size} ]\``, `${category.filter((cmd) => !cmd.description.hidden).map((cmd) => `\`${cmd.aliases[0]}\``).join(', ')}`)
        }
