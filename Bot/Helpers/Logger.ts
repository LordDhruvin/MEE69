...
export default class MageLogger {
    ...
    public debug(body: string, title: string = 'Debug'): void {
        console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(255, 69, 0) `[ ${title} ]`}\n${body}`)
    }
    ...
    public error(err: Error): void {
        console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(255, 0, 0) `[ ${err.name} ]`}\n${err.stack}`)
    }
    ...
    public warn(body: string, title: string = 'Warning') {
        console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(255, 255, 0) `[ ${title} ]`}\n${body}`)
    }
    ...
    public other(body: string, title: string = 'Other') {
        console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(112, 128, 144) `[ ${title} ]`}\n${body}`)
    }
    ...
    public success(body: string, title: string = 'Success') {
        console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(0, 255, 0) `[ ${title} ]`}\n${body}`)
    }
    ...
    public log(type: string, body: string, title: string) {
        ...
        switch (type.toLowerCase()) {
            case 'command':
                ...
                console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(0, 206, 209) `[ ${title} ]`}\n${body}`)
                break;
        
            case 'listener':
                if(!title) title = 'Listener'
                console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(153, 50, 204) `[ ${title} ]`}\n${body}`)
                break;
            
            case 'inhibitor':
                if(!title) title = 'Inhibitor'
                console.log(`${chalk.bold.rgb(186, 85, 211) `[ ${this._getFormattedTime()} ]`} - ${chalk.bold.rgb(233, 150, 122) `[ ${title} ]`}\n${body}`)
                break;
